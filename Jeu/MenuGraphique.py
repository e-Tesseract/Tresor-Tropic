############################################################################################
# développer par Hugo, Brian et Léo
# le programme permet de lancer le jeu en mode graphique
############################################################################################
...
def main_menu():
    ...
    background_image = pygame.transform.scale(
        background_image, (largeur_ecran, hauteur_ecran)
    )

    # Charger les images des boutons et les redimensionner
    button_width, button_height = int(largeur_fenetre * 0.20), int(
        hauteur_fenetre * 0.1
    )

    play_button_image = pygame.image.load("./images/Boutons/button_rect.png")
    play_button_image = pygame.transform.scale(
        play_button_image, (button_width, button_height)
    )

    Multijoueur_button_image = pygame.image.load("./images/Boutons/button_rect.png")
    Multijoueur_button_image = pygame.transform.scale(
        Multijoueur_button_image, (button_width, button_height)
    )

    save_button_image = pygame.image.load("./images/Boutons/button_rect.png")
    save_button_image = pygame.transform.scale(
        save_button_image, (button_width, button_height)
    )

    quit_button_image = pygame.image.load("./images/Boutons/button_rect.png")
    quit_button_image = pygame.transform.scale(
        quit_button_image, (button_width, button_height)
    )

    stats_button_image = pygame.image.load("./images/Boutons/button_rect.png")
    stats_button_image = pygame.transform.scale(
        stats_button_image, (button_width, button_height)
    )
    ...
    play_button_rect.center = (
        int(largeur_fenetre // 2),
        int((hauteur_fenetre // 2) * 0.30),
    )

    Multijoueur_button_rect = Multijoueur_button_image.get_rect()
    Multijoueur_button_rect.center = (
        int(largeur_fenetre // 2),
        int((hauteur_fenetre // 2) * 0.65),
    )
    ...
    quit_button_rect.center = (
        int(largeur_fenetre // 2),
        int((hauteur_fenetre // 2 * 1.35)),
    )

    stats_button_rect = stats_button_image.get_rect()
    stats_button_rect.center = (
        int(largeur_fenetre // 2),
        int((hauteur_fenetre // 2) * 1.70),
    )

    # Créer les surfaces de texte pour les boutons
    font = pygame.font.Font(
        "./images/BlackBeard/BlackBeard.otf", int(largeur_fenetre // 30)
    )
